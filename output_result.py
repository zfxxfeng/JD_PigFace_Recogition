...
directory='E:\\zfeng\\JD_pigface\\Recognize1\\prediction'
resize=[224,224]
filetypes=['.JPG']
...
with slim.arg_scope(resnet_arg_scope(is_training=False)):
    net, end_points = resnet_v2_50(X_test, 30,keep_prob=keep_prob1)
    net= tf.reshape(net, [-1, 30])#概率
    net_Probability=slim.softmax(net)
...
#若之前训练中断，可从中断处继续训练
    ...
    with open("end.csv","w",encoding='utf8',newline='') as csvfile:
        writer = csv.writer(csvfile)

        for i, s in enumerate(samples):
            image_str=s.split('\\')[-1]
            image_num=image_str.split('.')[0]#pig's num
            samples[i] = load_image(s)
            if resize:
                samples[i] = resize_image(samples[i], resize[0], resize[1])
            samples[i] = pil_to_nparray(samples[i])
            samples[i] /= 255.
            samples_image=samples[i].reshape(1,224,224,3)

            net_Probability_1=sess.run(net_Probability,feed_dict={X_test: samples_image,keep_prob1:1})
            print(net_Probability_1)
            for j in range(30):
                print(image_num)
                print(j+1)
                print( ("%.15f" % net_Probability_1[0][j]))
                writer.writerow([image_num,(j+1),("%.10f" % net_Probability_1[0][j])])
